...
class Fls_Log:
    ...
    def __init__(self, log_filepath=None, file_name=None, date_name=fmt_date(fmt=FMT_DATE)[:8],
                 handler_name='root', log_level=logging.DEBUG):
        if not log_filepath:
...
def fls_log(log_filepath=None, file_name='work', handler_name='root'):
    return Fls_Log(log_filepath=log_filepath, file_name=file_name, handler_name=handler_name)
